...
def plot_and_save(axes, fig, train_losses, eval_losses, scores, eval_x_axis):
    ...
    ax.set_title("Image Captioning Transformer - Flickr 8K")
    ax.set_xlabel("# batch")
    ax.set_ylabel("loss")
    ax.legend(loc='upper left')

    ax2.plot(eval_x_axis, scores, 'go-', label="BERT cosine similarity")
    ax2.set_ylabel("scores")
    ax2.legend(loc='upper right')
    fig.canvas.draw()
    fig.savefig("../figs/loss_BERT_8K.png")
...
def train(epochs=25, batch_size=128, lr=0.0003, num_layers=3):
    ...
    train_set = FDataset(
        ...
        capFilename=DATA_DIR + "/train.csv",
    ...
    try:
        for e in range(epochs):
            ...
            # save model weights and train & test loss for each epoch
